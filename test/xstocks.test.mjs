...
test("current: SPYx Solana — множитель строкой, пустой pending съедается", async () => {
  ...
  assert.equal(cur.pendingMultiplier, null); // newMultiplier: 0 = ничего не ждёт активации
  assert.equal(cur.activationDateTime, null);
  assert.equal(cur.reason, null);
});

test("current: ожидающий множитель пробрасывается", async () => {
...
test("history: 4 живых дивиденда SPYx (Ethereum) парсятся без float-потерь", async () => {
  ...
  const [last] = h.nodes; // API отдаёт новые сверху
  ...
  // самое старое событие: от единицы
...
test("history: пустая история (TSLAx Solana) — валидный пустой массив", async () => {
...
test("HTTP-ошибка классифицируется", async () => {
...
test("битая payloads отклоняются понятной ошибкой", async () => {
  await assert.rejects(
