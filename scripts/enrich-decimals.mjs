...
// The logic lives in src/registry/enrich.mjs (testability,: the write is atomic
...
// failure via process.exitCode — process.exit over a live undici socket
...
// Grammar = serve : both "--flag value" AND "--flag=value"; an empty value and
// a flag without a value — REFUSAL BEFORE any I/O (the "?? default" ate null
...
const readFlag = (name) => {
  ...
  if (eqIdx !== -1) {
    ...
    // "--api=--evil" used to escape into the runtime (a raw ENOENT/undici stack,
...
if (!badFlag) {
  ...
  // the registry is an OPERATOR file — a truncated/BOM/null/missing
  ...
  if (registryError !== null) {
    ...
    if (!res.ok) {
      ...
      // counter of missing decimals — BEFORE enrichment : after the mutation the list is
