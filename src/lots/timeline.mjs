 ...
 * (любой порядок; ИСКЛЮЧЕНИЕ — равные activationDateTime: такие события обязаны
 * приходить в цепочном порядке, обращённый порядок пары бьёт непрерывность —
 * ROUND7 №17, эмитенты мгновенными дублями не замечены), проверяет
 * НЕПРЕРЫВНОСТЬ ЦЕПОЧКИ (from[i+1] === to[i])
